...
def analyze_interference(codes, code_vectors, D, sample_size=10000, seed=42):
    ...
    Unterscheide k/K Überlappung (Anzahl gemeinsamer Komponentenwerte).
    """
    print("=" * 60)
    ...
    codes_i = sample_codes[tri_i]
    codes_j = sample_codes[tri_j]
    
    # Berechne k (Anzahl geteilter Komponenten)
    shared_counts = np.sum(codes_i == codes_j, axis=1)

    print(f"\n  k/K Überlappungs-Analyse (K=5):")
    print(f"  {'k':>2} | {'Anzahl':>10} | {'Anteil':>8} | {'mean':>8} | {'std':>8} | {'max':>8}")
    print(f"  ---+------------+----------+----------+----------+---------")

    stats_by_k = {}
    for k in range(6):
        mask = (shared_counts == k)
        k_sims = all_sims[mask]
        count = len(k_sims)
        if count > 0:
            m = k_sims.mean()
            s = k_sims.std()
            mx = k_sims.max()
            print(f"  {k:1d} | {count:10,d} | {count/len(all_sims)*100:7.2f}% | {m:8.4f} | {s:8.4f} | {mx:8.4f}")
            stats_by_k[k] = {"mean": m, "std": s, "max": mx, "count": count}
        else:
            print(f"  {k:1d} | {'0':>10} | {'0.00%':>8} | {'-':>8} | {'-':>8} | {'-' :>8}")

    # Kompatibilität zu altem Rückgabewert (verbunden = k>=1, unverbunden = k=0)
    connected_sims = all_sims[shared_counts >= 1]
    unconnected_sims = all_sims[shared_counts == 0]

    return {
        "connected_mean": connected_sims.mean() if len(connected_sims) > 0 else 0,
        "connected_max": connected_sims.max() if len(connected_sims) > 0 else 0,
        "unconnected_mean": unconnected_sims.mean() if len(unconnected_sims) > 0 else 0,
        "unconnected_max": unconnected_sims.max() if len(unconnected_sims) > 0 else 0,
...
def calculate_theoretical_d_krit(t=0.5, n_pairs=50_000_000, alpha=0.01):
    import math
    epsilon = alpha / n_pairs
    return math.ceil(2 * math.log(1 / epsilon) / (t ** 2))

def calculate_theoretical_capacity(D, t=0.5, alpha=0.01):
    import math
    ln_n = (D * (t**2)) / 4.0 + math.log(alpha) / 2.0
    return math.exp(ln_n)

def run_phase11(model, basis, test_codes, D=1024):
    print("\n" + "=" * 60 + "\nPHASE 11 – 1-Bit Quantisierung\n" + "=" * 60)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.eval()
    vecs = build_code_vectors(test_codes[:2000], basis, D)
    q_vecs = np.sign(vecs)
    cos_sims = np.sum(vecs * q_vecs, axis=1) / (np.linalg.norm(vecs, axis=1) * np.sqrt(D))
    print(f"  Empirisches gamma: {np.mean(cos_sims):.4f} (Theorie: {np.sqrt(2/np.pi):.4f})")
    print("  === Phase 11 abgeschlossen ===")

def run_phase12(model, basis, test_codes, D=1024, rates=(0.01, 0.05, 0.10)):
    print("\n" + "=" * 60 + "\nPHASE 12 – Satelliten-Robustheit (Bit-Flips)\n" + "=" * 60)
    rng = np.random.default_rng(42)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.eval()
    for p in rates:
        v = build_code_vectors(test_codes[:1000], basis, D)
        q = np.sign(v)
        mask = rng.random(q.shape) < p
        q[mask] *= -1.0
        with torch.no_grad():
            x = torch.tensor(q, dtype=torch.float32, device=device)
            y = torch.tensor(test_codes[:1000], dtype=torch.long, device=device)
            outputs = model(x)
            preds = torch.stack([o.argmax(dim=1) for o in outputs], dim=1)
            acc = (preds == y).all(dim=1).float().mean()
        print(f"  Bit-Flip Rate {p*100:>2.0f}%: Accuracy = {acc*100:6.2f}%")
    print("  === Phase 12 abgeschlossen ===")

def generate_orthogonal_basis_vectors(D, seed=42):
    rng = np.random.default_rng(seed)
    n_total = 55
    random_matrix = rng.standard_normal((D, n_total))
    q, r = np.linalg.qr(random_matrix)
    ortho_vecs = q.T
    basis = {}
    curr = 0
    comp_counts = {"A":10,"B":10,"C":20,"D":5,"E":10}
    for name, count in comp_counts.items():
        basis[name] = ortho_vecs[curr:curr+count]
        curr += count
    return basis
